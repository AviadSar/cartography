import torch
from torch.nn import Module, CrossEntropyLoss, MSELoss, Dropout, Linear, ReLU
...
class T5Pooler(Module):
    def __init__(self, config):
        super().__init__()
        self.dense = Linear(config.d_model, config.d_model)
        self.activation = ReLU()

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # We "pool" the model by simply taking the hidden state corresponding
        # to the last token.
        last_token_tensor = hidden_states[:, 0]
...
class AdaptedRobertaForSequenceClassification(RobertaForSequenceClassification):
    ...
    Examples::
        ...
        outputs = (
                  logits,) + outputs  # Modified from original `Transformers` since we need sequence output to summarize.
...
class AdaptedDebertaForSequenceClassification(DebertaForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
...
class AdaptedDebertaForMultipleChoice(DebertaForSequenceClassification):
    def __init__(self, config):
        super().__init__(config)
        output_dim = self.pooler.output_dim
        self.classifier = Linear(output_dim, 1)
...
class AdaptedT5ForSequenceClassification(T5PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = 3
        ...
        self.classifier = Linear(config.d_model, self.num_labels)
...
class AdaptedT5ForMultipleChoice(T5PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.t5 = T5EncoderModel(config)
        self.pooler = T5Pooler(config)
        self.dropout = Dropout(config.dropout_rate)
        self.classifier = Linear(config.d_model, 1)
...
class AdaptedXLNetForMultipleChoice(XLNetForMultipleChoice):
    def __init__(self, config):
        super().__init__(config)
...
class AdaptedElectraForMultipleChoice(ElectraForMultipleChoice):
    def __init__(self, config):
        super().__init__(config)
