...
def plot_data_map(dataframe: pd.DataFrame,
    ...
    fig.tight_layout()
    base_dir = os.path.basename(os.path.normpath(args.model_dir))
    filedir = f'{plot_dir}/{args.task_name}/{base_dir}/' if show_hist else f'figures/'
    filename = f'{title}_{model}.pdf' if show_hist else f'compact_{title}_{model}.pdf'
    if not os.path.exists(filedir):
        os.makedirs(filedir)
    fig.savefig(os.path.join(filedir, filename), dpi=300)
    logger.info(f"Plot saved to {os.path.join(filedir, filename)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plots_dir",
                        default="/cs/labs/roys/aviadsa/cartography/cartography/plots",
                        type=os.path.abspath,
    ...
    parser.add_argument("--filtering_output_dir",
                        "-f",
                        default="/cs/labs/roys/aviadsa/cartography/cartography/filtered_datasets",
                        type=os.path.abspath,
